...
const Vec3 SunlightComponentManager::getColour(uint index)
{
	return m_data.light_colour[index];
}

const float SunlightComponentManager::getLumen(uint index)
{
	return m_data.lumen[index];
}

const float SunlightComponentManager::getStarRadius(uint index)
{
