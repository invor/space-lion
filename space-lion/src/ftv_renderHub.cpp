...
bool Ftv_RenderHub::deleteScene()
{
	return false;
}

bool Ftv_RenderHub::setSceneParameters()
{
	return false;
}

Scene* Ftv_RenderHub::getScene(const int index)
{
...
void Ftv_RenderHub::setActiveScene(const int index)
{
	std::list<Scene>::iterator iter = sceneList.begin();
	for( int i=0; i < index; ++i) {++iter;}
	activeScene = &(*iter);
...
void Ftv_RenderHub::runVolumeTest()
{
	Mesh* geomPtr;
	Texture3D* volPtr;
	GLSLProgram* prgmPtr;
